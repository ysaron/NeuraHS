...
def populate_db():
    ...
    write_card_classes(card_classes)
    write_tribes(tribes)
    write_card_sets(card_sets)
...
def write_card_classes(classlist: list):
    """  """
...
def write_tribes(tribelist: list):
    """  """
    for t in tribelist:
...
def write_card_sets(setlist: list):
    """  """
    for s in setlist:
    ...
    card_set, created = CardSet.objects.get_or_create(name='unknown')
    if created:
        card_set.save()
